...
* [ ] `app/layout.tsx`
  ...
  * [ ] Add `metadata` + canonical + OG/Twitter
  * [ ] `<html lang dir>` from locale; skip link; shell (header/footer)
  * [ ] Render `<Providers>` inside `<body>`
...
* [ ] `app/providers.tsx` (client)
  ...
  * [ ] ThemeProvider (persist theme), QueryClient singleton, Toaster/Tooltip
  * [ ] `dir` propagation for RTL, `suppressHydrationWarning` as needed
  * [ ] **Acceptance:** no duplicate providers in Strict Mode
* [ ] `middleware.ts`

  * [ ] Detect locale (cookie or `Accept-Language`), set prefix, bypass static assets
  * [ ] Set cookie on client language switch
  * [ ] **Acceptance:** no loops; `/he/...` works; `/` rewrites to `/en` if needed
* [ ] `lib/i18n/config.ts` + `lib/i18n/index.ts`

  * [ ] Export `locales`, `defaultLocale`, `rtlLocales`; `isRTL()`
  * [ ] `t(key, locale)` stub pulling from page `_content/*.json`, safe fallback
  * [ ] **Acceptance:** missing keys don’t crash; logs dev warning only
* [ ] `lib/telemetry.ts`

  * [ ] Typed `track(event, payload)`: `cta_click`, `tool_opened`; no-op prod; console dev
  * [ ] **Acceptance:** TS errors on invalid event/payload
* [ ] `instrumentation.ts`

  * [ ] `export async function register()` logging trace markers (dev only)
  * [ ] **Acceptance:** Edge-safe; no runtime errors
* [ ] `tailwind.config.ts`

  * [ ] Content globs; dark `class`; shadcn preset; typography plugin; tokens (radii/spacing)
  * [ ] **Acceptance:** build passes; custom classes available
...
* [ ] **Refactor temporary AI Terminal tabs**
