...
check_server() {
    if curl -s -o /dev/null -w "%{http_code}" http://localhost:3001 | grep -q "200"; then
        ...
        echo -e "${YELLOW}⚠️  Dev server not responding on port 3001${NC}"
...
# Step 1: Check if server is running (should always be running on port 3001)
if ! check_server; then
    echo -e "${RED}✗ Dev server is not accessible on port 3001${NC}"
