...
/**
 * Visual Verification Enforcement Script
 * Ensures all UI changes are visually verified before task completion
 */
...
require('dotenv').config({ path: '.env.visual-verification' });
 ...
 * The server should ALWAYS be running on port 3001 in our dev environment
 */
function isDevServerRunning() {
  try {
    execSync('curl -s http://localhost:3001 > /dev/null 2>&1');
    return true;
  } catch {
    return false;
  }
 ...
 * In our setup, the server should always be running on port 3001
 */
async function ensureDevServer() {
  // Check if server is already running (which it should be)
  if (isDevServerRunning()) {
    log.success('Development server confirmed running on http://localhost:3001');
    return true;
  }
  
  // If not running, this is unexpected - warn the user
  log.warning('Dev server not detected on port 3001!');
  ...
  try {
    const psResult = execSync('ps aux | grep -E "next.*dev.*3001" | grep -v grep', { encoding: 'utf8' });
    if (psResult) {
      ...
      if (isDevServerRunning()) {
        log.success('Server is now responding on http://localhost:3001');
        return true;
      }
    }
  } catch {
  ...
  log.error('Dev server is not running on port 3001');
  log.info('Please run "pnpm dev" in another terminal to start the server');
  throw new Error('Dev server must be running on port 3001 for visual verification');
}

/**
 * Take screenshots using Playwright
 */
async function takeScreenshots(url = 'http://localhost:3001') {
 ...
 */
async function runAccessibilityChecks(url = 'http://localhost:3001') {
