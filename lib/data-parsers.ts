  ...
  lines.forEach((line, index) => {
    if (line.trim()) {
      ...
      if (columns.length >= 5) {
        ...
        if (
          highThreatKeywords.some(
            (keyword) => lowercaseNarrative.includes(keyword) || lowercaseAffiliation.includes(keyword)
          )
        ) {
...
function calculateConnectionStrength(actor1: ActorData, actor2: ActorData): number {
  ...
  const commonNarrativeWords = narrative1Words.filter((word) => narrative2Words.includes(word) && word.length > 4)
  ...
  const commonAffiliationWords = affiliation1Words.filter((word) => affiliation2Words.includes(word) && word.length > 3)
...
export async function loadMatrixData(): Promise<MatrixData> {
  try {
    // Load CSV data
    const csvResponse = await fetch("/i18n/FakeResistance_Actors_Archive_2025-08-29.csv")
    const csvData = await csvResponse.text()
    const actors = parseActorData(csvData)

    // Load JSON data
    const jsonResponse = await fetch("/i18n/cognitive_warfare_messages_multilingual.json")
    ...
    const networkNodes = actors.sort((a, b) => b.audienceNumber - a.audienceNumber).slice(0, 25) // Top 25 actors for network
