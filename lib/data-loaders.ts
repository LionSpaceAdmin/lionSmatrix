 ...
 */
// JSON is served from public/desigen_data at runtime; avoid compile-time imports
  ...
  headers.forEach((header, index) => {
      ...
      switch (key) {
        ...
        default:
          ...
          (actor as Record<string, string>)[key.replace(/[^a-z0-9]/g, '_')] = String(values[index] ?? '');
      }
    });
...
export function getRandomMessage(
  ...
  if (!randomMessage) return "Truth is pattern. AI sees it.";
  const langVal = (randomMessage as unknown as Record<string, unknown>)[language];
  return String((langVal as string) || randomMessage.en || "Truth is pattern. AI sees it.");
}
...
export async function loadMessages(): Promise<{ messages: MultilingualMessage[], languages: string[] }> {
  ...
  try {
    // Prefer JSON from public folder at runtime
    const jsonResp = await fetch('/desigen_data/cognitive_warfare_messages_multilingual.json');
    if (jsonResp.ok) {
      const jsonData = await jsonResp.json();
      const messagesArray = Array.isArray(jsonData) ? jsonData : ((jsonData as Record<string, unknown>).messages || []);
      ...
      console.log('📊 Loaded messages from JSON (public):', messages.length);
      return result;
    }
    // Fallback to CSV
    try {
  ...
  const headers = first.split(',').map(h => h.trim().toLowerCase());
    ...
    console.warn('⚠️ Unexpected error in loadMessages, returning fallback:', err);
