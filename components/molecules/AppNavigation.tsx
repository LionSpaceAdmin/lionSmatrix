      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center space-x-2 p-2 rounded terminal-button-hover focus:outline-none focus:ring-2 focus:ring-terminal-cyan/20"
            aria-expanded={userMenuOpen}
            aria-haspopup="menu"
            aria-label="User menu"
          >
            <div className="w-8 h-8 bg-terminal-cyan/20 rounded border border-terminal-cyan flex items-center justify-center">
              <span className="text-xs font-mono text-terminal-cyan" aria-hidden="true">OP</span>
            </div>
          ...
          {userMenuOpen && (
            <div 
              className="absolute right-0 mt-2 w-48 bg-terminal-secondary border border-terminal-border rounded shadow-lg z-50"
              role="menu"
            ...
            >
              <div className="py-1">
                <Link
                  href="/app/settings"
                  className="block px-4 py-2 text-sm text-terminal-text font-mono hover:bg-terminal-bg focus:bg-terminal-bg focus:outline-none focus:ring-1 focus:ring-terminal-cyan/30"
                  role="menuitem"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'ArrowDown') {
                      ...
                      if (nextItem && 'focus' in nextItem) {
                        nextItem.focus();
                      }
                    } else if (e.key === 'Escape') {
                      setUserMenuOpen(false);
                      (document.querySelector('[aria-haspopup="menu"]') as HTMLElement)?.focus();
                    }
                  }}
                >
                  SETTINGS
                ...
                <button
                  className="block w-full text-left px-4 py-2 text-sm text-terminal-red font-mono hover:bg-terminal-bg focus:bg-terminal-bg focus:outline-none focus:ring-1 focus:ring-terminal-cyan/30"
                  role="menuitem"
                  onClick={() => {
                    setUserMenuOpen(false);
